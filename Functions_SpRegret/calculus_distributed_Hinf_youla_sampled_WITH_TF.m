function [K_opt, Q_opt, objective] = calculus_distributed_Hinf_youla_sampled_WITH_TF(sys, delays_matrix,options)
...
% for k = 1:options.N_tf+1
%     Q_sdp{k} = sdpvar(sys.m,sys.p,'full').*Q_mask(:,:,k);
% end
...
% % Find indices where omega_vector is around pi/4 and 2*pi - pi/4 (within +/- 0.1)
% omega_targets = [pi/4, 2*pi - pi/4];
% tolerance = 0.1;

% % Convert omega_vector to radians (since it's normalized to [0,1])
% omega_vector_rad = 2 * pi * omega_vector;

% indices_around_targets = [];
% for target = omega_targets
%     idx = find(abs(omega_vector_rad - target) <= tolerance);
%     indices_around_targets = [indices_around_targets, idx];
% end

% % Remove duplicates and sort
% indices_around_targets = unique(indices_around_targets);
