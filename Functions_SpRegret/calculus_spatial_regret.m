function [K_opt, Phis_or_Q, objective] = calculus_spatial_regret(sys,oracle_LFT,delays_matrix,options)
clear yalmip;
...
for i=1:n_z_vectors
    ...
    Delta_tf_t = trim_matrix(Delta_tf_t,options.trim_tol);

    LMI_t = [1*eye(sys.n_z), Tzw_tf_t; Tzw_tf_t', lambda*eye(sys.n_w) + Delta_tf_t];
    constraints = [constraints, LMI_t >= 0];
end
